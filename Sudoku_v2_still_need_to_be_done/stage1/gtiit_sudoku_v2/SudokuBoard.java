...
public class SudokuBoard {
    ...
    public boolean containsInBox(int row, int col, int value) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (board.get(boxStartRow + i).get(boxStartCol + j).getValue() == value) {
                    return true;
                }
            }
        }
    ...
    private boolean Some_Row_ok( int row ){
        for( int col = 0 ; col <= SIZE - 1 ; col++ ){
            if( board.get(row).get(col).getValue() > 9 || board.get(row).get(col).getValue() < 1)
                return false;
        } 
        for( int number = 1 ; number <= 9 ; number++ ){
            int flat = 0;
            for( int col = 0 ; col <= SIZE - 1 && flat == 0 ; col++ ){
                if( number == board.get(row).get(col).getValue() )
                    flat = 1;
            }
            if( flat == 0)
                return false;
