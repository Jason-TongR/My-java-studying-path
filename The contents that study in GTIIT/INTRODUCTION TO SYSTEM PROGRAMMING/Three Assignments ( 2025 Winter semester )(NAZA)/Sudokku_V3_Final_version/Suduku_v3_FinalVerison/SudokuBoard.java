...
public class SudokuBoard {
    ...
    public boolean validValueInCell(int row, int col) {
       assert row >= 0 && row < SIZE : "row must between 0 and 8";
        ...
        if( board[row][col].getValue() == 0 )
            return true;    //because 0 is empty , always valid.
        else{               //for not 0 elments should follow follow's rules.
    ...
    public boolean check_whether_borad_full(){
        for( int row = 0 ; row < SIZE ; row++ ){
            for( int col = 0 ; col < SIZE ; col++ ){
                if(board[row][col].getValue() == 0){
                    return false;
