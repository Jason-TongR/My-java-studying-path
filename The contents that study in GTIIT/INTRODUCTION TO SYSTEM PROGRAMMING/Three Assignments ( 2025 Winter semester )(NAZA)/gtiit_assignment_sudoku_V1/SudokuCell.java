...
public class SudokuCell {
    ...
    public void setValue(int value) {
        assert (isFixed == false) : "the cell must empty";
        assert ( (value >=1) && (value <= 9) ) : "your enter value must between 1 to 9";
        this.value = value;//set value.
        isFixed = true;     //fixed it.
    }
    ...
    public void makeFixed() {
        assert (isFixed == false) : "the cell must not fixed";
        assert ( (value >= 1) && (value <= 9) ) : "the value is from 1 to 9 (not 0)";
        isFixed = true;//now the cell is fixed.
    }
